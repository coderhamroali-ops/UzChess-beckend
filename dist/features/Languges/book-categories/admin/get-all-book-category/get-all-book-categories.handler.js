...
Object.defineProperty(exports, "GetAllBookCategoriesHandler", {
    enumerable: true,
    get: function() {
        return GetAllBookCategoriesHandler;
    }
});
...
let GetAllBookCategoriesHandler = class GetAllBookCategoriesHandler {
    async execute(query) {
        const currentPage = query.page ?? 1;
        const take = query.size ?? 3;
        const skip = (currentPage - 1) * take;
        const totalCount = await _bookcategoryentities.Entitybook.count();
        const totalPages = Math.ceil(totalCount / take);
        const hasNext = currentPage < totalPages;
        const hasPrevious = currentPage > 1;
        const data = await _bookcategoryentities.Entitybook.find({
            select: {
                title: true
            },
            take: take,
...
], GetAllBookCategoriesHandler.prototype, "execute", null);
GetAllBookCategoriesHandler = _ts_decorate([
